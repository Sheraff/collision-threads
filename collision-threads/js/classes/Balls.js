 ...
 * @param {number} length 
 */
function makeBuffer(type, _length) {
	const bytes = type.BYTES_PER_ELEMENT
	const length = _length * bytes
	return {
		length,
		type: type.name,
	}
...
export default class Balls {
	...
	initBuffers(memory) {
		...
		this.initViewsFromBuffers(memory.buffer)
	}

	/**
	 * @param {WebAssembly.Memory.buffer} memory 
	 */
	initViewsFromBuffers(memory, buffers = this.buffers) {
		let offset = 0
		Object.entries(buffers).forEach(([key, { length, type }]) => {
			this[key] = new globalThis[type](memory, offset, length)
			offset += length
		})
	}
